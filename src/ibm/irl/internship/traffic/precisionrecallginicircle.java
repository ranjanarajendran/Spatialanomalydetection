...
public class precisionrecallginicircle {
	...
	public static void main(String args[]) throws IOException{
		...
		Integer maxnumcirc = Integer.parseInt(args[3]);
	...
	public static int findseattleneighbors(Double centrex, Double centrey,Double radius){
		...
		for(int i =0; i < numpoints; i++){
			...
			/*Double latitudeY1=Math.abs(geo[i][1]*((Math.PI)/180));
			Double longitudeX1=Math.abs(geo[i][0]*((Math.PI)/180));
			
			Double latitudeY2=Math.abs(centrey*((Math.PI)/180));
			Double longitudeX2=Math.abs(centrex*((Math.PI)/180));
				
			/*Double objX1 = Math.abs(earthR * Math.cos(latitudeY1)*Math.cos(longitudeX1));
			Double objY1 = Math.abs(earthR * Math.cos(latitudeY1)*Math.sin(longitudeX1));
			
			Double objX2 = Math.abs(earthR * Math.cos(latitudeY2)*Math.cos(longitudeX2));
			Double objY2 = Math.abs(earthR * Math.cos(latitudeY2)*Math.sin(longitudeX2));*/
			
			/*ouble dLat = (latitudeY2-latitudeY1);
			Double dLon = (longitudeX2-longitudeX1);

			Double a = Math.sin(dLat/2) * Math.sin(dLat/2) +
			        Math.sin(dLon/2) * Math.sin(dLon/2) * Math.cos(latitudeY1) * Math.cos(latitudeY2); 
			Double c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a)); 
			Double distance = c * earthR / 100 ;*/
			
			
			//Double distance = Math.acos(Math.sin(latitudeY1)*Math.sin(latitudeY2) + Math.cos(latitudeY1)*Math.cos(latitudeY2) *Math.cos(longitudeX2-longitudeX1)) * earthR * 1000;
