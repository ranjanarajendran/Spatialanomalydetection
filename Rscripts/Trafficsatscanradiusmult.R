...
#if(args[5] == 1){
    output.col<-read.table(args[4], header=TRUE,col.names=c("V1","V2","V3","V4","V5","V6","V7"))
#}else{
...
output.col<-output.col[order(V6),]
...
circles(centrex[numcircles],centrey[numcircles], radii[numcircles], col = NA, border = "black")
circles(centrex[numcircles-1],centrey[numcircles-1], radii[numcircles-1], col = NA, border = "red")
dev.off()
