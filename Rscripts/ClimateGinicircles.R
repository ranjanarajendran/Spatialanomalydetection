...
#  ClimateGinicircles.R
#  
#
#  Created by Ranjana Rajendran on 12/20/12.
...
seattleginicirclerectangles<-read.table(args[7], header=TRUE,col.names=c("V1","V2","V3","V4"))

attach(seattleginicirclerectangles)
leftx <- V1
lefty <- V2
rightx <- V3
righty <- V4
...
NAsatscanneighborresult<-read.table(args[8], header=TRUE,col.names=c("V1","V2","V3","V4","V5","V6","V7","V8","V9","V10","V11"))

attach(NAsatscanneighborresult)
circleids<-V1
centrex <- V2
centrey <- V3
radii <- V4
...
for(i in 1:numcircles){
    if(circleids[i] %in% relevantcircleids){
